# 👋 Harley Bonfatti
## Customer Success | E-commerce | Operations

**Location:** Praia Grande, SP 🇧🇷 | **Status:** Disponível para Home Office 🏠

---

## 🎯 Sobre Mim

Profissional com **6+ anos de experiência** em operações, e-commerce e customer success. Especialista em transformar caos operacional em processos escaláveis com impacto direto no faturamento.

Minha força: **documentar cada passo** e **medir tudo em números** (ROI, margem, cancelamento, receita, etc).

---

## 📊 Meu Portfólio

Tenho 3 projetos documentados com resultados reais:

### 📈 **[CASO 1: Sistema de Precificação E-commerce](CASO_ESTUDO_1_PRECIFICACAO.md)**
- **Problema:** 18% das vendas saindo com prejuízo
- **Solução:** Sistema inteligente de precificação (automações low-code, SQL e scripts) + avisador de margem via Supabase
- **Resultado:** Eliminadas perdas | **+150% na margem**
- **Tecnologia:** SQL, automações low-code (Zapier/Make), Supabase, APIs de marketplace, conhecimento básico em Python | Análise de dados
- **📥 PDF:** [Download Completo](docs/CASO_ESTUDO_1_PRECIFICACAO_Harley.pdf)

### 🏥 **[CASO 2: Automação CRM para Clínicas](CASO_ESTUDO_2_CRM_CLINICAS.md)**
- **Problema:** 25% de cancelamento de pacientes
- **Solução:** CRM automatizado com Supabase + Zapier
- **Resultado:** **-68% cancelamento** (25% → 8%) | **+40% receita**
- **Tecnologia:** Supabase | PostgreSQL | Webhooks | Automação

### 💰 **[CASO 3: Transformação Digital Marketplace](CASO_ESTUDO_3_ECOMMERCE.md)** ⭐ *Meu melhor*
- **Problema:** Marketplace desorganizado, muitas devoluções, operacional lenta
- **Solução:** Otimização completa (precificação, estoque, fulfillment, CS)
- **Resultado:** 
  - **+82% receita** em 6 meses
  - **-67% devoluções**
  - **-55% tempo operacional**
  - **6.800 pedidos/mês** com 2 pessoas
- **Impacto:** R$ 137.000/mês de receita adicional
- **Tecnologia:** SQL, automações low-code (Zapier/Make), conhecimento básico em Python | Automação | Process Optimization

---

## 🛠️ Stack Técnico

- **Linguagens / Ferramentas:** SQL; conhecimento básico em Python (scripts simples) — foco em ferramentas low-code/no-code (Zapier, Make)
- **Plataformas:** Supabase (PostgreSQL), Zapier/Make, Shopee, Mercado Livre, TikTok Shop, Amazon
- **Ferramentas:** Excel (avançado), CRM, ERP, Git (básico)
- **Competências:** Precificação dinâmica, Automação (low-code/no-code), Análise de dados (SQL), Customer Success

---


---

## 💼 Experiência

### **Fox Pet Shop** (2025 - Presente)
- **Cargo:** Assistente de E-commerce
- **Responsabilidades:**
  - Suporte nas operações de 6.000+ pedidos/mês em múltiplos marketplaces
  - Auxiliar na otimização de precificação e margem por plataforma
  - Acompanhar e reduzir devoluções e reclamações
  - Suportar a implementação de automações para escalabilidade
  - Trabalhar com ferramentas low-code/no-code e SQL para relatórios e integrações

### Experiências Anteriores
- Consultoria em E-commerce
- Gerência de CRM e automação
- Operações em marketplace
- Customer Success (SaaS e B2B)

---

## 🎓 Competências Principais

✅ **Operacional:** Processos, escalabilidade, eficiência, análise de gargalos
✅ **E-commerce:** Marketplaces (Shopee, Mercado Livre, TikTok Shop, Amazon), precificação, fulfillment
✅ **Automação:** N8N, MANYCHAT, Webhooks, CRM, Google Sheets
✅ **Dados:** SQL, análise, reporting, métricas (ROI, LTV, CAC, etc)
✅ **Customer Success:** Retenção, satisfação, suporte, relacionamento

---

## 📈 Minhas Métricas

| Projeto | Impacto | Tempo |
|---------|---------|-------|
| Precificação | +150% margem | 2 meses |
| CRM Clínicas | -68% cancelamento | 1 mês |
| E-commerce | +82% receita | 6 meses |

---

## 🔗 Links Úteis

- **LinkedIn:** [linkedin.com/in/harleybonfatti](https://linkedin.com/in/harleybonfatti)
- **Email:** [harleyrogerio11@gmail.com]
- **Telefone:** (11) 99608-4893

---

## 💡 O Que Procuro

**Posição:** Customer Success Manager | Operations Manager | CS Operations | Success Lead

**Empresa ideal:**
- Remota 🏠
- SaaS em crescimento
- Ambiente de dados/métricas
- Que valorize otimização e escalabilidade

**O que trago:**
- 6+ anos de experiência comprovada
- Documentação de tudo (cases, processos, metrics)
- Mentalidade de impacto (número em tudo)
- Disposição de aprender rápido

---

## 📞 Quer conversar?

Estou sempre disponível para:
- Conversar sobre operações e e-commerce
- Discutir desafios de customer success
- Trocar experiências sobre scaling
- Informational interviews

**Envie um email ou conecte no LinkedIn!**

---

*Portfólio criado em 2026 | Última atualização: Setembro 2026*

**Status:** 🟢 Ativo e disponível para oportunidades remotas
